...
a) Backward pass equations for an      layer (linear transformation + bias).
...
def main():
    ...
    # Check if the max of the softmax is below a threshold
    # If below then show the plot the sample (uncertain predictions):
    print('Show samples where model is uncertain:')
    inputs_train, inputs_valid, inputs_test, target_train, target_valid, target_test = load_data("toronto_face.npz")
    predictions = softmax(nn_forward(model, inputs_test)["y"])
    threshold = 0.5

    counter = 0
    print('Show the sample where the prediction is uncertain:')
    for idx, prediction in enumerate(predictions):
        max_class_prob = np.max(prediction)
        if max_class_prob < threshold:
            img = np.reshape(inputs_test[idx], (48, 48))
            print('Actual Output:', np.argmax(target_test[idx]))
